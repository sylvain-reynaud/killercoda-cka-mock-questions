...
# 1. Check for the crds.yaml file and its content
if [ ! -f "$CRDS_FILE" ]; then
    echo "Error: File '$CRDS_FILE' not found."
    exit 1
fi

# Check for some of the key cert-manager CRDs in the file
if ! grep -q "certificates.cert-manager.io" "$CRDS_FILE" || ! grep -q "issuers.cert-manager.io" "$CRDS_FILE" || ! grep -q "clusterissuers.cert-manager.io" "$CRDS_FILE"; then
    echo "Error: '$CRDS_FILE' does not seem to contain the list of cert-manager CRDs."
    exit 1
fi
