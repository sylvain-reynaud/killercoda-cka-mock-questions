...
kubectl create secret generic mysql-pass --from-literal=password=my-secret-pw -n wordpress
...
kubectl wait --for=condition=available --timeout=600s deployment/wordpress-mysql -n wordpress
kubectl wait --for=condition=available --timeout=600s deployment/wordpress -n wordpress

# Scale the deployment to 3 replicas as a starting point
