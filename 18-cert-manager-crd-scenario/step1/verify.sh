...
# Check that the default kubectl output format (table) was used
if ! head -n 1 "$RESOURCES_FILE" | grep -q "NAME" || ! head -n 1 "$RESOURCES_FILE" | grep -q "CREATED AT"; then
    echo "Error: '$RESOURCES_FILE' does not appear to use the default kubectl output format. The header is missing or incorrect."
    exit 1
fi
